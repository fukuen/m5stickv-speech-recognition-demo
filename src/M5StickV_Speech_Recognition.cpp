...
int i2s_dma_irq(void *ctx)
{
    ...
    if(i2s_start_flag)
    {
        ...
        if(g_index)
        {
            i2s_receive_data_dma(I2S_DEVICE_0, &g_rx_dma_buf[g_index], frame_mov * 2, DMAC_CHANNEL2);
            g_index = 0;
        ...
        {
            i2s_receive_data_dma(I2S_DEVICE_0, &g_rx_dma_buf[0], frame_mov * 2, DMAC_CHANNEL2);
            g_index = frame_mov * 2;
...
M5StickVSpeechRecognizer::M5StickVSpeechRecognizer(){
...
int
M5StickVSpeechRecognizer::begin()
{
    //io_mux_init
    ...
    dmac_init();
    dmac_set_irq(DMAC_CHANNEL2, i2s_dma_irq, NULL, 3);
    i2s_receive_data_dma(I2S_DEVICE_0, &g_rx_dma_buf[0], frame_mov * 2, DMAC_CHANNEL2);
...
int
M5StickVSpeechRecognizer::record(uint8_t keyword_num, uint8_t model_num)
{
...
int
M5StickVSpeechRecognizer::recognize()
{
    u8 res;
    u32 dis;
...
int
M5StickVSpeechRecognizer::addVoiceModel(uint8_t keyword_num, uint8_t model_num, const int16_t *voice_model, uint16_t frame_num)
{
...
int
M5StickVSpeechRecognizer::print_model(uint8_t keyword_num, uint8_t model_num)
{
...
uint8_t M5StickVSpeechRecognizer::save_mdl(uint16_t *v_dat, uint32_t addr)
{
    u16 i, num;
    u16 frame_index;
get_noise1:
...
uint8_t M5StickVSpeechRecognizer::spch_recg(uint16_t *v_dat, uint32_t *mtch_dis)
{
        u16 i;
    u32 ftr_addr;
    u32 min_dis;
